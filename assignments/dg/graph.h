...
template <typename N, typename E> class Graph {
 private:
  struct Node {
    ...
    // constructor
    explicit Node(const N &node) : node_ptr_{make_shared<N>(node)} {}
  ...
  struct Edge {
    ...
    Edge(const shared_ptr<N> &src, const shared_ptr<N> &dst, const E &w)
        : src_ptr_{src}, dst_ptr_{dst}, weight_ptr_{make_shared<E>(w)} {}

    // necessary for std::find() and graph == operator to work
    friend bool operator==(const Edge &a, const Edge &b) {
      return *(a.src_ptr_) == *(b.src_ptr_) && *(a.dst_ptr_) == *(b.dst_ptr_) &&
 ...
 public:
  ...
  const_reverse_iterator crbegin() const {
    return const_reverse_iterator{edges_.crbegin()};
  }
  ...
  // constructors//
  Graph<N, E>(void) {}
  Graph<N, E>(typename std::vector<N>::const_iterator start,
  ...
  // destructor
  // the vectors and smart pointers will deallocate themselves
  ~Graph() = default;
